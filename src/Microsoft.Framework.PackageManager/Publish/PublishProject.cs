...
namespace Microsoft.Framework.PackageManager.Publish
{
    public class PublishProject
    {
        ...
        private bool PrunePackages(PublishRoot root)
        {
            ...
            var frameworks = root.Runtimes.Select(r => r.Framework).ToList();
            ...
            foreach (var project in root.Projects)
            {
                ...
                {
                    // TODO: Add runtime id check
                    if (!frameworks.Contains(target.TargetFramework))
                    {
                        continue;
                    }
        ...
        private bool UpdateLockFile(PublishRoot root)
        {
            ...
            var projects = root.Projects.Where(p => p.IsPackage).ToList();
            var tasks = new Task<bool>[projects.Count];
            for (int i = 0; i < projects.Count; i++)
            {
                ...
                restoreCommand.RestoreDirectory = project.TargetPath;
