...
namespace Microsoft.Framework.PackageManager
{
    public class RestoreCommand
    {
        ...
        public async Task<bool> Execute()
        {
            ...
            {
                Reports.Error.WriteLine("{0}Errors in {1}".Red().Bold(), Environment.NewLine, category.Key);
            ...
            if (settings != null)
            {
                var configFiles = settings.GetNuGetConfigFiles();
        ...
        private async Task<bool> Execute(string restoreDirectory)
        {
            try
            {
                ...
                if (_installCount > 0)
                {
                    InformationMessages.GetOrAdd("Installed:", _ => new List<string>()).Add($"{_installCount} packages to {packagesDirectory}");
                    _installCount = 0;
                }
